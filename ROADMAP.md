...
## 1. Pinned gravity wells (orbital sandbox)

The held-`G` well, but persistent: press `W` to pin an attractor at the cursor,
`Shift+W` to pin a repeller, and a clear key (e.g. `Shift+R` or double-`W`).
Multiple wells -> binary systems, slingshots, orbit painting with trails.

- Sim: `Vec<Well>` field, applied each substep like the held well (same
  Plummer softening; strength maybe half of WELL_STRENGTH per pinned well).
- Render: draw a small marker per well (ring or dot).
- HUD: "Wells: 3 (W pins, Shift+W repels)".
- Unlocks an `orbits` preset: gravity 0, spawn off, a few pinned wells, trails.
- Tests: pinned well accelerates particles; clear removes; marker count.

## 2. Bullet-time explosions
...
- More presets once #1/#4 exist: `orbits`, `mandala` (kaleidoscope+fireworks).
